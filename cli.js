...
const GET_Router = (route, response, body, headers) => {
  ...
  if (isRoot) {
    ...
    response.write(`<!doctype html>
        ...
        <hr />
        <h3 id="postman"> <a href="${"data:text/json;charset=utf-8," + encodeURIComponent(utils.generatePostmanC(API))}" download="fakeapi-v1.3.2.postman_collection.json">Download Postman Collection</a></h3>
        <h3 > <a href="${"data:text/json;charset=utf-8," + encodeURIComponent(utils.generatePostmanEnvObject(API, 'LOCAL'))}"
         download="fakeapi_v1.3.2_local.postman_environment.json">Download Local Environment Variables</a></h3> 
...
server.listen(PORT, () => {
  console.log(`======================`);
  console.log(`Simple Fake API v1.3.2`);
  console.log(`======================`);
